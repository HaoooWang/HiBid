...
class DQNAgent(object):
  ...
  def __init__(self,
    ...
    with tf.device(self.tf_device):
      ...
      self.state_ph = tf.placeholder(
          tf.float32, [None, self.state_dim], name='state')
      self.Lambda_ph = tf.placeholder(
          tf.float32, [None, 1], name='lambda')
      self.next_state_ph = tf.placeholder(
        tf.float32, [None, self.state_dim], name='next_state')
      self.action_ph = tf.placeholder(
        tf.int32, [None, 1], name='current_action')
      self.reward_ph = tf.placeholder(
        tf.float32, [None, 1], name='reward_ph')
      self.done_ph = tf.placeholder(
        tf.float32, [None, 1], name='done_ph')
      self.next_action_ph = tf.placeholder(
        tf.int32, [None, 1])
  ...
  def _build_networks(self):
    ...
    self._q_argmax = tf.expand_dims(tf.argmax(self._net_outputs.q_values, axis=1), axis= 1, name = 'next_action')
    self._replay_net_outputs = self.online_convnet(self.state_ph, self.Lambda_ph)
    # self._replay_net_outputs = self.online_convnet(tf.cast(self.replay_buffer.new_batch_data["state"], dtype = tf.float32),
    #                                                self.replay_buffer.new_batch_data["Lambda"])
    self._replay_next_target_net_outputs = self.target_convnet(self.next_state_ph, self.Lambda_ph)

    # self._replay_next_target_net_outputs = self.target_convnet(
    #     tf.cast(self.replay_buffer.new_batch_data["next_state"], dtype = tf.float32), self.replay_buffer.new_batch_data["Lambda"])
  ...
  def _network_template(self, state, Lambda):
    ...
    r10 = tf.layers.batch_normalization(tf.concat([state, Lambda], 1), axis=-1, momentum=0.99,
                                        epsilon=0.001,
                                        center=True,
                                        scale=True,
  ...
  #
  # def _build_replay_buffer(self, replay_buffer):
  #
  #     self._replay.states = self.replay_buffer.new_batch_data["state"]
  #     self._replay.next_states = self.replay_buffer.new_batch_data["next_state"]
  #     self._replay.rewards = self.replay_buffer.new_batch_data["rewards"]
  #     self._replay.actions = self.replay_buffer.new_batch_data["actions"]
  #     self._replay.terminals = self.replay_buffer.new_batch_data["done"]
  #     self._replay.Lambda = self.replay_buffer.new_batch_data["Lambda"]
  #     self._replay.next_action = self.replay_buffer.new_batch_data["next_action"],
  ...
  def _build_target_q_op(self):
    ...
    return self.reward_ph + self.cumulative_gamma * replay_next_qt_max * (
        1. - tf.cast(self.done_ph, tf.float32))
  ...
  def _build_train_op(self):
    ...
    replay_action_one_hot = tf.one_hot(
        self.action_ph, self.num_actions, 1., 0., name='action_one_hot')
