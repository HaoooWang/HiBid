...
parser.add_argument('--Number_stores',default=4000,type=int)#系统中商家的个数
parser.add_argument('--Number_days',default=3,type=int)#正式统计的天数
# parser.add_argument('--High_cycle_days',default=3,type=int)#高层期天数
parser.add_argument('--Number_pvs',default=100000,type=int)#每天的PV个数
parser.add_argument('--Number_type_of_store',default=8,type=int)#商家类型个数
parser.add_argument('--Number_of_channels',default=3,type=int)#渠道个数
parser.add_argument('--Number_stores_for_pv',default=10,type=int)#每次竞价参与的最大商家数量
...
def main(args):
    Number_stores = args.Number_stores  # 门店数量
    Number_days = args.Number_days  # 天数
    # High_cycle_days = args.High_cycle_days #高层周期天数
    Number_pvs = args.Number_pvs  # pv流量数
    Number_type_of_store = args.Number_type_of_store  # 门店类型数
    Number_of_channels = args.Number_of_channels  # 渠道数
    Number_stores_for_pv = args.Number_stores_for_pv  # pv门店数：每个pv流量下参与竞价的商家的数量
    ...
    store_CTR = np.zeros(Number_stores)  # 初始化，每个门店的平均点击率
    store_CVR = np.zeros(Number_stores)  # 初始化，每个门店的平均成交率
    store_GMV = np.zeros(Number_stores)  # 初始化，每个门店的平均成交额
    store_type = np.zeros(Number_stores)  # 初始化，每个门店的类型
    # 最终的数据储存
    # saved_day = []
    saved_state_h = []
    saved_action_b_h = []
    ...
    # all_hist_gmv = np.zeros(Number_days, Number_stores)  # 初始化，所有历史的成交额（每天每个门店）
    # 对于每个渠道来说，每个商家每天在该渠道的成交额，成本，成交数，点击数，竞价成功数，竞价总次数，竞价成功率，点击率，成交率，预算，ROI，真实ROI
    all_hist_gmv_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_cost_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_gmvn_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_clickn_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_winn_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_bidn_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_winr_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_ctr_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_cvr_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_bud_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_roi_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_real_roi_in_chan = np.zeros([Number_days, Number_stores, Number_of_channels])
    all_hist_chan_pv = np.zeros([Number_days, Number_of_channels])    #每天各渠道总pv
    all_hist_roi = np.zeros([Number_days, Number_stores])    #每天每个商家的ROI
    attend_bid_time = np.zeros(Number_stores)  # 初始化，各门店竞价成功次数？？？
    for store in range(Number_stores):  # 每个门店的平均点击率，平均成交率，平均成交额，门店类型
        store_CTR[store] = random.uniform(0, 1)
        store_CVR[store] = random.uniform(0, 1)
        store_GMV[store] = random.uniform(0, Max_GMV)    # 单次成交数最好是多大
    ...
    for day in range(Number_days):    # 先验数据生成
        ...
        today_gmv_in_chan = np.zeros([Number_stores, Number_of_channels])
        today_cost_in_chan = np.zeros([Number_stores, Number_of_channels])
        today_gmvn_in_chan = np.zeros([Number_stores, Number_of_channels])
        today_clickn_in_chan = np.zeros([Number_stores, Number_of_channels])
        today_winn_in_chan = np.zeros([Number_stores, Number_of_channels])
        today_bidn_in_chan = np.zeros([Number_stores, Number_of_channels])
        today_chan_pv = np.zeros(Number_of_channels)  # 当天各渠道总pv
        # 每个商家为每个渠道分配当天的预算和ROI
        store_budget = np.zeros(Number_stores)    # 初始化，每个商家当天的预算
        store_budget_leave = np.zeros(Number_stores)    # 初始化，商家竞价时的剩余预算，最初等于商家当天的预算
        store_ROI = np.zeros(Number_stores)    # 初始化，每个商家当天的ROI
        store_budget_in_chan = np.zeros([Number_stores, Number_of_channels])    # 初始化，每个商家在每个渠道当天的预算参考值
        store_gmv_in_chan = np.zeros([Number_stores, Number_of_channels])  # 初始化，每个商家在每个渠道当天的GMV参考值
        store_ROI_in_chan = np.zeros([Number_stores, Number_of_channels])    # 初始化，每个商家在每个渠道当天的ROI参考值
        ...
        for pv in range(Number_pvs):    #遍历当天每一条pv流量
            ...
            bool_click = 0  # 是否点击
            bool_convert = 0  # 是否转化
            store_prefer = np.zeros(Number_type_of_store)  # 初始化，维度为门店类型数量的先验，代表当前用户对各类型门店的喜好程度（喜欢烧烤、面食等情况）
            ...
            stores_for_pv = np.zeros(Number_stores_for_pv)  # 初始化，维度为pv门店数，代表参与当前pv流量竞价的商家
            store_bid = np.zeros(Number_stores_for_pv)  # 初始化，维度为pv门店数，代表参与竞价的商家出的价
            ...
            flag = 0
            for store in range(Number_stores_for_pv):
                stores_for_pv[store] = np.random.randint(Number_stores)
            ...
            max_index = store_bid_list.index(max(store_bid_list))  # 出价最高的门店在参与竞价的门店中的位置
            temp = store_bid.copy()
            # np.delete(temp, max_index)
            temp = np.delete(temp, max_index)
            store_budget_leave[int(stores_for_pv[max_index])] -= np.max(temp)  # 获胜门店的预算减去第二高的出价作为成本
            attend_bid_time[int(stores_for_pv[max_index])] += 1  # 获胜门店竞价成功次数加一
            ...
            # 开始用户行为判定
            random_ctr_value = np.random.rand()
            random_cvr_value = np.random.rand()
            current_gmv = 0
            all_ctr = store_CTR[int(stores_for_pv[max_index])] * user_ctr * store_prefer[
                int(store_type[int(stores_for_pv[max_index])])] * Number_type_of_store  # 最终用户点击率=门店点击率*用户点击率*用户对该门店类型的喜爱程度*门店类型数
            all_cvr = store_CVR[int(stores_for_pv[max_index])] * user_cvr * store_prefer[
                int(store_type[int(stores_for_pv[max_index])])] * Number_type_of_store  # 最终用户转化率=门店转化率*用户转化率*用户对该门店类型的喜爱程度*门店类型数
            if random_ctr_value <= np.random.normal(all_ctr, 0.05):  # 用户是否点击与是否成交（使用以最终点击率、成交率和成交额为均值的正态分布）
                ...
                if random_cvr_value <= np.random.normal(all_cvr, 0.05):
                    ...
                    all_gmv = store_GMV[int(stores_for_pv[max_index])] * user_gmv * store_prefer[int(store_type[
                        int(stores_for_pv[max_index])])] * Number_type_of_store  # 最终用户成交额=门店成交额*用户成交额*用户对该门店类型的喜爱程度*门店类型数
            ...
            # 修改商家的平均CTR,CVR和GMV
            for store in range(Number_stores_for_pv):
                if store == max_index:  # 针对获胜门店
                    if bool_click == 1:  # 点击了，新的门店点击率={[门店点击率*（100+门店获胜次数）]+1}/（101+门店获胜次数）
                        ...
                        if bool_convert == 1:  # 成交了，新的门店成交率={[门店转化率*（100+门店获胜次数）]+1}/（101+门店获胜次数）
                            ...
                            store_GMV[int(stores_for_pv[store])] = ((store_GMV[int(stores_for_pv[store])] * (
                                    100 + attend_bid_time[int(stores_for_pv[store])])) + current_gmv) / (
                                                                      101 + attend_bid_time[int(stores_for_pv[store])])
                            # 成交了，新的门店成交额={[门店成交额*（100+门店获胜次数）]+本次成交额}/（101+门店获胜次数）
                        else:  # 没成交，新的门店成交率={[门店转化率*（100+门店获胜次数）]}/（101+门店获胜次数）
                            store_CVR[int(stores_for_pv[store])] = ((store_CVR[int(stores_for_pv[store])] * (
                                    100 + attend_bid_time[int(stores_for_pv[store])]))) / (
                                                                      101 + attend_bid_time[int(stores_for_pv[store])])
                    else:  # 没点击，新的门店点击率={[门店点击率*（100+门店获胜次数）]}/（101+门店获胜次数）
                        store_CTR[int(stores_for_pv[store])] = ((store_CTR[int(stores_for_pv[store])] * (
                                100 + attend_bid_time[int(stores_for_pv[store])]))) / (
                                                                  101 + attend_bid_time[int(stores_for_pv[store])])
        # 一天结束，所有商家记录当天的历史效益。
    ...
    #                   np.zeros(Number_days),np.zeros(Number_days),np.zeros(Number_days),np.zeros(Number_days),
    #                   np.zero(1),np.zero(1),))
    # state_h = np.zeros([Number_days*Number_stores,Number_days*13*Number_of_channels+Number_days*13+2])
    # 低层状态信息：
    ...
    #                   np.zeros(13),)
    # state_l = np.zeros([Number_days*Number_pvs*Number_stores_for_pv,
    ...
    state_l_dim = 12+14+15*(Number_of_channels-1)+13+Number_type_of_store+Number_of_channels    #
    # 初始化高层状态，动作，奖励、成本，下一个状态，是否终止
    state_h = np.zeros([Number_stores, Number_days, state_h_dim])
    action_b_h = np.zeros([Number_stores, Number_days, Number_of_channels])
    action_g_h = np.zeros([Number_stores, Number_days, Number_of_channels])
    reward_h = np.zeros([Number_stores, Number_days])
    next_state_h = np.zeros([Number_stores, Number_days, state_h_dim])
    terminal_h = np.zeros([Number_stores, Number_days])
    for day in range(Number_days):
        # 初始化低层状态，动作，奖励、成本、GMV、商家ROI（作为约束的值），下一个状态，是否终止
        # Number_stores行，Number_of_channels列个空列表[]
        state_l = [[[] for j in range(Number_of_channels)] for i in range(Number_stores)]
        action_l = [[[] for j in range(Number_of_channels)] for i in range(Number_stores)]
        reward_l = [[[] for j in range(Number_of_channels)] for i in range(Number_stores)]
        cost_l = [[[] for j in range(Number_of_channels)] for i in range(Number_stores)]
        gmv_l = [[[] for j in range(Number_of_channels)] for i in range(Number_stores)]
        roi_l = [[[] for j in range(Number_of_channels)] for i in range(Number_stores)]
        next_state_l = [[[] for j in range(Number_of_channels)] for i in range(Number_stores)]
        terminal_l = [[[] for j in range(Number_of_channels)] for i in range(Number_stores)]
        # 初始化，当天每个门店在每个渠道上的成交额，成本，成交数，点击数，竞价成功数，参与竞价次数
        today_gmv_in_chan = np.zeros([Number_stores, Number_of_channels])
        today_cost_in_chan = np.zeros([Number_stores, Number_of_channels])
        today_gmvn_in_chan = np.zeros([Number_stores, Number_of_channels])
        today_clickn_in_chan = np.zeros([Number_stores, Number_of_channels])
        ...
        for pv in range(Number_pvs):    # 遍历当天每一条pv流量
            ...
            stores_for_pv = np.zeros(Number_stores_for_pv)  # 初始化，维度为pv门店数，代表参与当前pv流量竞价的商家
            store_bid = np.zeros(Number_stores_for_pv)  # 初始化，维度为pv门店数，代表参与竞价的商家出的价
            type_stores_for_pv = np.zeros(Number_stores_for_pv)
            ctr_for_pv = np.zeros(Number_stores_for_pv)
            cvr_for_pv = np.zeros(Number_stores_for_pv)
            gmv_for_pv = np.zeros(Number_stores_for_pv)
            today_gmv_for_pv = np.zeros(Number_stores_for_pv)
            today_cost_for_pv = np.zeros(Number_stores_for_pv)
            today_gmvn_for_pv = np.zeros(Number_stores_for_pv)
            today_clickn_for_pv = np.zeros(Number_stores_for_pv)
            today_winn_for_pv = np.zeros(Number_stores_for_pv)
            today_bidn_for_pv = np.zeros(Number_stores_for_pv)
            today_winr_for_pv = np.zeros(Number_stores_for_pv)
            today_ctr_for_pv = np.zeros(Number_stores_for_pv)
            today_cvr_for_pv = np.zeros(Number_stores_for_pv)
            today_bud_for_pv = np.zeros(Number_stores_for_pv)
            today_roi_for_pv = np.zeros(Number_stores_for_pv)
            today_bud_cur_for_pv = np.zeros(Number_stores_for_pv)
            today_roi_cur_for_pv = np.zeros(Number_stores_for_pv)
            random_value1 = np.random.rand()
            random_value2 = np.random.rand()
            random_value3 = np.random.rand()
            # 决定用户点击率
            ...
            for store in range(Number_stores_for_pv):
                ...
                current_channel_state_l = np.concatenate((
                    ...
                    np.array([store_CTR[int(store_pv)], store_CVR[int(store_pv)], store_GMV[int(store_pv)]]),
                ...
                # 默认奖励暂时为竞价失败的-1
                reward_l[int(store_pv)][current_channel].append(-1)
                # 默认成本暂时为竞价失败的0
        ...
        all_hist_gmv_in_chan = np.concatenate((all_hist_gmv_in_chan, today_gmv_in_chan.reshape((1, Number_stores, Number_of_channels))))
        all_hist_gmv_in_chan = np.delete(all_hist_gmv_in_chan, 0, 0)
        all_hist_cost_in_chan = np.concatenate((all_hist_cost_in_chan, today_cost_in_chan.reshape((1, Number_stores, Number_of_channels))))
        all_hist_cost_in_chan = np.delete(all_hist_cost_in_chan, 0, 0)
        all_hist_gmvn_in_chan = np.concatenate((all_hist_gmvn_in_chan, today_gmvn_in_chan.reshape((1, Number_stores, Number_of_channels))))
        all_hist_gmvn_in_chan = np.delete(all_hist_gmvn_in_chan, 0, 0)
        all_hist_clickn_in_chan = np.concatenate((all_hist_clickn_in_chan, today_clickn_in_chan.reshape((1, Number_stores, Number_of_channels))))
        all_hist_clickn_in_chan = np.delete(all_hist_clickn_in_chan, 0, 0)
        all_hist_winn_in_chan = np.concatenate((all_hist_winn_in_chan, today_winn_in_chan.reshape((1, Number_stores, Number_of_channels))))
        all_hist_winn_in_chan = np.delete(all_hist_winn_in_chan, 0, 0)
        all_hist_bidn_in_chan = np.concatenate((all_hist_bidn_in_chan, today_bidn_in_chan.reshape((1, Number_stores, Number_of_channels))))
        ...
        today_clickn_in_chan_temp = today_clickn_in_chan * 1
        today_clickn_in_chan_temp[today_clickn_in_chan_temp == 0] = 1
        all_hist_winr_in_chan = np.concatenate((all_hist_winr_in_chan, (today_winn_in_chan / today_bidn_in_chan_temp).reshape((1, Number_stores, Number_of_channels))))
        all_hist_winr_in_chan = np.delete(all_hist_winr_in_chan, 0, 0)
        all_hist_ctr_in_chan = np.concatenate((all_hist_ctr_in_chan,(today_clickn_in_chan / today_winn_in_chan_temp).reshape((1, Number_stores, Number_of_channels))))
        all_hist_ctr_in_chan = np.delete(all_hist_ctr_in_chan, 0, 0)
        all_hist_cvr_in_chan = np.concatenate((all_hist_cvr_in_chan,(today_gmvn_in_chan / today_clickn_in_chan_temp).reshape((1, Number_stores, Number_of_channels))))
        all_hist_cvr_in_chan = np.delete(all_hist_cvr_in_chan, 0, 0)
        all_hist_bud_in_chan = np.concatenate((all_hist_bud_in_chan, store_budget_in_chan.reshape((1, Number_stores, Number_of_channels))))
        all_hist_bud_in_chan = np.delete(all_hist_bud_in_chan, 0, 0)
        all_hist_roi_in_chan = np.concatenate((all_hist_roi_in_chan, store_ROI_in_chan.reshape((1, Number_stores, Number_of_channels))))
        all_hist_roi_in_chan = np.delete(all_hist_roi_in_chan, 0, 0)
        today_cost_in_chan_temp = today_cost_in_chan * 1
        today_cost_in_chan_temp[today_cost_in_chan_temp == 0] = 1
        all_hist_real_roi_in_chan = np.concatenate(
            (all_hist_real_roi_in_chan, (today_gmv_in_chan / today_cost_in_chan_temp).reshape((1, Number_stores, Number_of_channels))))
        ...
        all_hist_roi = np.concatenate(
            (all_hist_roi, store_ROI.reshape((1, Number_stores))))
        ...
        for store in range(Number_stores):
            ...
            reward_h[store, day] = 0 if today_winn_temp==0 else today_clickn_temp/today_winn_temp
        ...
        np.savez('trajectory_low_dataForMaddpgSearch_day' + str(day+1) + '.npz', state_l_day=state_l_day, action_l_day=action_l_day,
